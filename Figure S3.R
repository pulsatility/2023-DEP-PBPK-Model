# Krais 2018: 300 ug/m^3 exposred for 3 hours wearing clothes. 
# Weschler 2015: 250 ug/m^3 exposed for 6 hours body top exposed to air.

# Simulation results of the human DEP model for 4 different DEP exposure scenarios as in the Weschler or Krais studies by using the chemical-specific parameter values in Jeong study.

# The default values are for Weschler no hood. 
# To predict Weschler hood, change QP to 0*5*60, and FXskin to 0.82
# To predict Krais hood, change QP to 0*5*60, and FXskin to 0.04
# To predict Krais no hood, change QP to 5*60, and FXskin to 0.13

library(deSolve)
library(ggplot2)
library(ggthemes)

# Import model code containing parameter and ODEs
#source("Model_DEP_12012023.R")
source("Model_DEP.R")
parameters["P_gut_plasma_DEP"]<-0.38 # default 5.92
parameters["P_liver_plasma_DEP"]<-10 # default 5.95
parameters["P_gonads_plasma_DEP"]<-0.82 # default 3.26
parameters["P_skin_plasma_DEP"]<-1 # default 7.29
parameters["P_fat_plasma_DEP"]<-1.2 # default 64.42
parameters["P_restbody_plasma_DEP"]<-0.41 # default 1.66
parameters["P_gut_plasma_MEP"]<-0.65 # default 1.88
parameters["P_liver_plasma_MEP"]<-2.18 # default 1.89
parameters["P_gonads_plasma_MEP"]<-0.47 # default 1.16
parameters["P_skin_plasma_MEP"]<-0.5 # default 2.25
parameters["P_fat_plasma_MEP"]<-0.45 # default 17.42
parameters["P_restbody_plasma_MEP"]<-0.8 # default 0.75
parameters["fu_DEP"]<-0.294 # default 0.30
parameters["CLGint"]<-9.91 # default 991
parameters["CLLint"]<-16.42 #default 1642


##### -------------Figure S3A-3D: To simulate Weschler -------------- ##### 

exposure_duration <- 6
times <- seq(0, exposure_duration, by = 0.01)
CI_Air <- 0.25
parameters["CI"]<-CI_Air

##### ------ Figure S3A-3B: To simulate Weschler hood ------- ##### 
QP=0			                              # ventilation rate (L/h) 
FXskin =0.82                               # fraction of skin exposed
parameters["Skin_area_expo"]<-Skin_area*FXskin

# Run simulation - during exposure
parameters.1<-parameters
out <- lsoda(y = state, times = times, func = DEPMEP, parms = parameters.1, jacfunc = NULL)

# Run simulation - after exposure
parameters.2 <- parameters.1
parameters.2["CI"]<-c(0) # set air concentration to 0

out.dataframe <- as.data.frame(out)
state.time.2<-as.numeric(tail(out.dataframe,n=1))[-1]

state.time.2.vector <- c( Agut  =state.time.2[1],
                          Aliver  = state.time.2[2],
                          Afat  = state.time.2[3],
                          Agonads  = state.time.2[4],
                          Askin  = state.time.2[5],
                          Aabsorbed = state.time.2[6],
                          Arestbody  = state.time.2[7],
                          Aplasma  = state.time.2[8],
                          Aexhaled = state.time.2[9],
                          Ainhaled = state.time.2[10],
                          Agut_MEP  = state.time.2[11],
                          Aliver_MEP  = state.time.2[12],
                          Afat_MEP  = state.time.2[13],
                          Agonads_MEP  = state.time.2[14],
                          Askin_MEP  = state.time.2[15],
                          Arestbody_MEP  = state.time.2[16],
                          Aplasma_MEP  = state.time.2[17],
                          Aurine_MEP  = state.time.2[18],
                          Aglucuronidated_MEP =state.time.2[19],
                          Agut_lumen=state.time.2[20]                  
)

times.2 <- seq(exposure_duration,60,by = 0.01)
out.2 <- lsoda(y = state.time.2.vector , times = times.2, func = DEPMEP, parms = parameters.2, jacfunc=NULL )
out.final <-rbind.data.frame(out,out.2[-1,])
cre_adjusted_Auriune_MEP <- out.final[,"dAurine_MEP"]/creatinine_prod_rate #ug/g

time <- out.final[,1]
concentration <- log10(cre_adjusted_Auriune_MEP)
dat <- data.frame(time,concentration)
dat$time <- round(dat$time,2)
dat$group <- "orgin"

# Load Weschler 2015 datasets - Hood
wes.data.hood.se<-read.table("Weschler hood - mean and se.txt",sep = "\t",col.names = c("time","UrineMEP","se_concentration","se_time"),
                             fill = TRUE,strip.white = TRUE,na.strings = "#NA")
wes.data.hood.se$ymin_conc <- log10(wes.data.hood.se$UrineMEP-wes.data.hood.se$se_concentration)
wes.data.hood.se$ymax_conc <- log10(wes.data.hood.se$UrineMEP+wes.data.hood.se$se_concentration)

wes.data.hood.se$xmin_t <- wes.data.hood.se$time-wes.data.hood.se$se_time
wes.data.hood.se$xmax_t <- wes.data.hood.se$time+wes.data.hood.se$se_time

# Plot Figure S3A - Urine MEP
p.wes.hood <-ggplot(wes.data.hood.se)+
  geom_errorbar(aes(x=time,y=log10(UrineMEP),ymin=ymin_conc, ymax=ymax_conc), 
                width=1)+
  geom_errorbarh(aes(x=time,y=log10(UrineMEP),xmin=xmin_t, xmax=xmax_t))+
  geom_point(data =wes.data.hood.se,aes(x=time, y=log10(UrineMEP)) ,shape=1,size=2)+
  geom_line(data=dat,aes(x=time, y=concentration),color="blue",size=1)+
  theme_classic()+ylab("Log10 Urinary MEP/Creatinine (µg/g)")+xlab("Time (h)")+
  scale_x_continuous(breaks = c(0,10,20,30,40,50,60))+
  ylim(c(0,4))+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))
p.wes.hood

# Plot Figure S3B - plasma DEP/plasma MEP
p.wes.hood.plasma <-ggplot(out.final)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,9]/Vplasma)),color="red",size=1)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,18]/Vplasma)),color="dark green",size=1)+
  theme_classic()+ylab("Log10 Plasma Concentration (µg/L)")+xlab("Time (h)")+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))+
  ylim(c(-4,1.5))+
  scale_x_continuous(breaks = c(0,10,20,30,40,50,60))
p.wes.hood.plasma


##### ------ Figure S3C-3D: To simulate Weschler no hood ------- #####
QP=5*60			                             # ventilation rate (L/h) 
FXskin =0.91                               # fraction of skin exposed
parameters["Skin_area_expo"]<-Skin_area*FXskin

# Run simulation - during exposure
parameters.1<-parameters
out <- lsoda(y = state, times = times, func = DEPMEP, parms = parameters.1, jacfunc = NULL)

# Run simulation - after exposure
parameters.2 <- parameters.1
parameters.2["CI"]<-c(0) # set air concentration to 0

out.dataframe <- as.data.frame(out)
state.time.2<-as.numeric(tail(out.dataframe,n=1))[-1]

state.time.2.vector <- c( Agut  =state.time.2[1],
                          Aliver  = state.time.2[2],
                          Afat  = state.time.2[3],
                          Agonads  = state.time.2[4],
                          Askin  = state.time.2[5],
                          Aabsorbed = state.time.2[6],
                          Arestbody  = state.time.2[7],
                          Aplasma  = state.time.2[8],
                          Aexhaled = state.time.2[9],
                          Ainhaled = state.time.2[10],
                          Agut_MEP  = state.time.2[11],
                          Aliver_MEP  = state.time.2[12],
                          Afat_MEP  = state.time.2[13],
                          Agonads_MEP  = state.time.2[14],
                          Askin_MEP  = state.time.2[15],
                          Arestbody_MEP  = state.time.2[16],
                          Aplasma_MEP  = state.time.2[17],
                          Aurine_MEP  = state.time.2[18],
                          Aglucuronidated_MEP =state.time.2[19],
                          Agut_lumen=state.time.2[20]                  
)

times.2 <- seq(exposure_duration,60,by = 0.01)
out.2 <- lsoda(y = state.time.2.vector , times = times.2, func = DEPMEP, parms = parameters.2, jacfunc=NULL )
out.final <-rbind.data.frame(out,out.2[-1,])
cre_adjusted_Auriune_MEP <- out.final[,"dAurine_MEP"]/creatinine_prod_rate #ug/g

time <- out.final[,1]
concentration <- log10(cre_adjusted_Auriune_MEP)
dat <- data.frame(time,concentration)
dat$time <- round(dat$time,2)
dat$group <- "orgin"

## Load Weschler 2015 datasets - No hood
wes.data.nohood.se<-read.table("Weschler no hood - mean and se.txt",sep = "\t",col.names = c("time","UrineMEP","se_concentration","se_time"),
                               fill = TRUE,strip.white = TRUE,na.strings = "#NA")
wes.data.nohood.se$ymin_conc <- log10(wes.data.nohood.se$UrineMEP-wes.data.nohood.se$se_concentration)
wes.data.nohood.se$ymax_conc <- log10(wes.data.nohood.se$UrineMEP+wes.data.nohood.se$se_concentration)

wes.data.nohood.se$xmin_t <- wes.data.nohood.se$time-wes.data.nohood.se$se_time
wes.data.nohood.se$xmax_t <- wes.data.nohood.se$time+wes.data.nohood.se$se_time

# Plot Figure S3C - Urine MEP
p.wes.nohood <-ggplot(wes.data.nohood.se)+
  geom_errorbar(aes(x=time,y=log10(UrineMEP),ymin=ymin_conc, ymax=ymax_conc), 
                width=1)+
  geom_errorbarh(aes(x=time,y=log10(UrineMEP),xmin=xmin_t, xmax=xmax_t))+
  geom_point(data =wes.data.nohood.se,aes(x=time, y=log10(UrineMEP)) ,shape=1,size=2)+
  geom_line(data=dat,aes(x=time, y=concentration,group=group),color="blue",size=1)+
  theme_classic()+ylab("Log10 Urinary MEP/Creatinine (µg/g)")+xlab("Time (h)")+
  scale_x_continuous(breaks = c(0,10,20,30,40,50,60))+
  ylim(c(0,4))+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))
p.wes.nohood

# Plot Figure S3D - plasma DEP/plasma MEP
p.wes.nohood.plasma <-ggplot(out.final)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,9]/Vplasma)),color="red",size=1)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,18]/Vplasma)),color="dark green",size=1)+
  theme_classic()+ylab("Log10 Plasma Concentration (µg/L)")+xlab("Time (h)")+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))+
  ylim(c(-4,1.5))+
  scale_x_continuous(breaks = seq(0,200,10))
p.wes.nohood.plasma


##### -------------Figure S3E-3H: To simulate Krais -------------- ##### 

exposure_duration=3
times <- seq(0, exposure_duration, by = 0.1)
CI_Air <- 0.3
parameters["CI"]<-CI_Air


##### -------------Figure S3E-3F: To simulate Krais hood -------------- ##### 
QP=0			                              # ventilation rate (L/h) 
FXskin =0.04                               # fraction of skin exposed
parameters["Skin_area_expo"]<-Skin_area*FXskin

# Run simulation - during exposure
parameters.1<-parameters
out <- lsoda(y = state, times = times, func = DEPMEP, parms = parameters.1, jacfunc = NULL)

# Run simulation - after exposure
parameters.2 <- parameters.1
parameters.2["CI"]<-c(0) # set air concentration to 0

out.dataframe <- as.data.frame(out)
state.time.2<-as.numeric(tail(out.dataframe,n=1))[-1]

state.time.2.vector <- c( Agut  =state.time.2[1],
                          Aliver  = state.time.2[2],
                          Afat  = state.time.2[3],
                          Agonads  = state.time.2[4],
                          Askin  = state.time.2[5],
                          Aabsorbed = state.time.2[6],
                          Arestbody  = state.time.2[7],
                          Aplasma  = state.time.2[8],
                          Aexhaled = state.time.2[9],
                          Ainhaled = state.time.2[10],
                          Agut_MEP  = state.time.2[11],
                          Aliver_MEP  = state.time.2[12],
                          Afat_MEP  = state.time.2[13],
                          Agonads_MEP  = state.time.2[14],
                          Askin_MEP  = state.time.2[15],
                          Arestbody_MEP  = state.time.2[16],
                          Aplasma_MEP  = state.time.2[17],
                          Aurine_MEP  = state.time.2[18],
                          Aglucuronidated_MEP =state.time.2[19],
                          Agut_lumen=state.time.2[20]                  
)

times.2 <- seq(exposure_duration,60,by = 0.1)
out.2 <- lsoda(y = state.time.2.vector , times = times.2, func = DEPMEP, parms = parameters.2, jacfunc=NULL )
out.final <-rbind.data.frame(out,out.2[-1,])
cre_adjusted_Auriune_MEP <- out.final[,"dAurine_MEP"]/creatinine_prod_rate #ug/g

time <- out.final[,1]
concentration <- log10(cre_adjusted_Auriune_MEP)
dat <- data.frame(time,concentration)
dat$time <- round(dat$time,2)
dat$group <- "orgin"

#Load Krais 2018 datasets - Hood
krais.data.hood.se<-read.table("Krais hood - mean and se.txt",sep = "\t",col.names = c("time","UrineMEP","se_concentration","se_time"),
                               fill = TRUE,strip.white = TRUE,na.strings = "#NA")
krais.data.hood.se$ymin_conc <- log10(krais.data.hood.se$UrineMEP-krais.data.hood.se$se_concentration)
krais.data.hood.se$ymax_conc <- log10(krais.data.hood.se$UrineMEP+krais.data.hood.se$se_concentration)

krais.data.hood.se$xmin_t <- krais.data.hood.se$time-krais.data.hood.se$se_time
krais.data.hood.se$xmax_t <- krais.data.hood.se$time+krais.data.hood.se$se_time

# Plot Figure S3E - Urine MEP
p.krais.hood <-ggplot(krais.data.hood.se)+
  geom_errorbar(aes(x=time,y=log10(UrineMEP),ymin=ymin_conc, ymax=ymax_conc), 
                width=1)+
  geom_errorbarh(aes(x=time,y=log10(UrineMEP),xmin=xmin_t, xmax=xmax_t))+
  geom_point(data =krais.data.hood.se,aes(x=time, y=log10(UrineMEP)) ,shape=1,size=2)+
  geom_line(data=dat,aes(x=time, y=concentration),color="blue",size=1)+
  theme_classic()+ylab("Log10 Urinary MEP/Creatinine (µg/g)")+xlab("Time (h)")+
  scale_x_continuous(breaks = c(0,10,20,30,40,50,60))+
  ylim(c(-0.5,3))+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))

p.krais.hood

# Plot Figure 3F - plasma DEP/plasma MEP
p.krais.hood.plasama <-ggplot(out.final)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,9]/Vplasma)),color="red",size=1)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,18]/Vplasma)),color="dark green",size=1)+
  theme_classic()+ylab("Log10 Plasma Concentration (µg/L)")+xlab("Time (h)")+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))+
  ylim(c(-5.5,0))+
  scale_x_continuous(breaks = c(0,10,20,30,40,50,60))

p.krais.hood.plasama


##### -------------Figure S3G-3H: To simulate Krais no hood -------------- ##### 
QP=5*60			                              # ventilation rate (L/h) 
FXskin =0.13                               # fraction of skin exposed
parameters["Skin_area_expo"]<-Skin_area*FXskin

# Run simulation - during exposure
parameters.1<-parameters
out <- lsoda(y = state, times = times, func = DEPMEP, parms = parameters.1, jacfunc = NULL)

# Run simulation - after exposure
parameters.2 <- parameters.1
parameters.2["CI"]<-c(0) # set air concentration to 0

out.dataframe <- as.data.frame(out)
state.time.2<-as.numeric(tail(out.dataframe,n=1))[-1]

state.time.2.vector <- c( Agut  =state.time.2[1],
                          Aliver  = state.time.2[2],
                          Afat  = state.time.2[3],
                          Agonads  = state.time.2[4],
                          Askin  = state.time.2[5],
                          Aabsorbed = state.time.2[6],
                          Arestbody  = state.time.2[7],
                          Aplasma  = state.time.2[8],
                          Aexhaled = state.time.2[9],
                          Ainhaled = state.time.2[10],
                          Agut_MEP  = state.time.2[11],
                          Aliver_MEP  = state.time.2[12],
                          Afat_MEP  = state.time.2[13],
                          Agonads_MEP  = state.time.2[14],
                          Askin_MEP  = state.time.2[15],
                          Arestbody_MEP  = state.time.2[16],
                          Aplasma_MEP  = state.time.2[17],
                          Aurine_MEP  = state.time.2[18],
                          Aglucuronidated_MEP =state.time.2[19],
                          Agut_lumen=state.time.2[20]                  
)

times.2 <- seq(exposure_duration,60,by = 0.01)
out.2 <- lsoda(y = state.time.2.vector , times = times.2, func = DEPMEP, parms = parameters.2, jacfunc=NULL )
out.final <-rbind.data.frame(out,out.2[-1,])
cre_adjusted_Auriune_MEP <- out.final[,"dAurine_MEP"]/creatinine_prod_rate #ug/g

time <- out.final[,1]
concentration <- log10(cre_adjusted_Auriune_MEP)
dat <- data.frame(time,concentration)
dat$time <- round(dat$time,2)
dat$group <- "orgin"


#Load Krais 2018 datasets - no Hood
krais.data.nohood.se<-read.table("Krais no hood - mean and se.txt",sep = "\t",col.names = c("time","UrineMEP","se_concentration","se_time"),
                                 fill = TRUE,strip.white = TRUE,na.strings = "#NA")
krais.data.nohood.se$ymin_conc <- log10(krais.data.nohood.se$UrineMEP-krais.data.nohood.se$se_concentration)
krais.data.nohood.se$ymax_conc <- log10(krais.data.nohood.se$UrineMEP+krais.data.nohood.se$se_concentration)

krais.data.nohood.se$xmin_t <- krais.data.nohood.se$time-krais.data.nohood.se$se_time
krais.data.nohood.se$xmax_t <- krais.data.nohood.se$time+krais.data.nohood.se$se_time

# Plot Figure S3G - Urine MEP
p.krais.nohood <-ggplot(krais.data.nohood.se)+
  geom_errorbar(aes(x=time,y=log10(UrineMEP),ymin=ymin_conc, ymax=ymax_conc), 
                width=1)+
  geom_errorbarh(aes(x=time,y=log10(UrineMEP),xmin=xmin_t, xmax=xmax_t))+
  geom_point(data =krais.data.nohood.se,aes(x=time, y=log10(UrineMEP)) ,shape=1,size=2)+
  geom_line(data=dat,aes(x=time, y=concentration),color="blue",size=1)+
  theme_classic()+ylab("Log10 Urinary MEP/Creatinine (µg/g)")+xlab("Time (h)")+
  scale_x_continuous(breaks = c(0,10,20,30,40,50,60))+
  ylim(c(-0.5,3.5))+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))

p.krais.nohood

# Plot Figure 3H - plasma DEP/plasma MEP
p.krais.nohood.plasama <-ggplot(out.final)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,9]/Vplasma)),color="red",size=1)+
  geom_line(data=out.final,aes(x=out.final[,1], y=log10(out.final[,18]/Vplasma)),color="dark green",size=1)+
  theme_classic()+ylab("Log10 Plasma Concentration (µg/L)")+xlab("Time (h)")+
  theme(axis.title.x = element_text(size = 12),
        axis.text.x = element_text(size = 12,colour="black"),
        axis.text.y = element_text(size = 12,colour="black"))+
  ylim(c(-5,1))+
  scale_x_continuous(breaks = c(0,10,20,30,40,50,60))

p.krais.nohood.plasama

